...
def assert_user(actual: UserSchema, expected: UserSchema):
    """
    Проверяет, что данные пользователя в ответе соответствуют ожидаемым данным.

    :param actual: Данные пользователя из ответа API.
...
def assert_get_user_response(get_user_response: GetUserResponseSchema, create_user_response: CreateUserResponseSchema):
    """
    Проверяет, что ответ на получение пользователя соответствует данным, полученным при создании пользователя.
