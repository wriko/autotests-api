...
print(create_user_response.status_code)       # выводит код состояния ответа на экран.
print(f"Данные нового пользователя: {create_user_response_data}")  # выводит ответ в формате JSON на экран.
...
update_user_payload = {  # создаёт словарь `update_user_payload` с данными для обновления информации о пользователе.
  "email": get_random_email(),  # использует функцию `get_random_email` для генерации нового случайного адреса электронной почты.
  "password": "string",
  "lastName": "111",
  "firstName": "222",
  "middleName": "333"
...
update_user_response = client.patch(f"/users/{user_id}", json=update_user_payload)  # отправляет PATCH-запрос на URL, содержащий идентификатор пользователя, с обновлёнными данными и сохраняет ответ в переменную `update_user_response`.
update_user_response_data = update_user_response.json() # преобразует ответ в формате JSON в словарь Python и сохраняет его в переменную `update_user_response_data`.
print(create_user_response.status_code)       # выводит код состояния ответа на экран.
print(f"Обновленные данные о пользователе: {update_user_response_data}") # выводит код состояния ответа и ответ в формате JSON на экран.
