...
async def echo(websocket: ServerConnection):  # определяет асинхронную функцию `echo`, которая принимает объект `websocket` типа `ServerConnection`
    async for message in websocket:  # асинхронно обрабатывает входящие сообщения от клиента в цикле
        print(f"Получено сообщение: {message}")  # выводит полученное сообщение в консоль
        response = f"Сервер получил сообщение, {message}!"  # формирует ответное сообщение, добавляя к полученному сообщению префикс "Сервер получил сообщение, "
        await websocket.send(response)  # асинхронно отправляет ответное сообщение клиенту
        print(f"Сообщение: {message} отправлено клиенту")  # выводит сообщение о том, что ответ отправлен клиенту
